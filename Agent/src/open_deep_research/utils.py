...
async def tavily_search(
    queries: List[str],
    max_results: Annotated[int, InjectedToolArg] = 5, # InjectedToolArg는 "이 매개변수는 AI가 직접 제공(AI한테는 숨겨서 보여줌)하지 않고, 시스템이 자동으로 주입한다"는 의미 (LangChain을 통해)
    topic: Annotated[Literal["general", "news", "finance"], InjectedToolArg] = "general", # Literal["general", "news", "finance"]는 Python의 타입 힌트로, 정확히 이 세 개 문자열 값만 허용한다는 의미
    ...
    """ # 사실 여기서, max_results, topic의 경우 InjectedToolArg로 하였으니 description에서도 숨기는게 맞음.
    ...
    for response in search_results:
        for result in response['results']:
            url = result['url']
            if url not in unique_results: # 동일 url에서 동일한 결과는 제거하는 Logic
                unique_results[url] = {**result, "query": response['query']} 
    configurable = Configuration.from_runnable_config(config)
    max_char_to_include = 50_000  # 웹페이지 요약을 위해 AI 모델에 보낼 텍스트의 최대 길이 NOTE: This can be tuned by the developer. This character count keeps us safely under input token limits for the latest models.
    # _는 1000단위 표시자로 쓰임 
...
async def summarize_webpage(model: BaseChatModel, webpage_content: str) -> str:
    try:
        summary = await asyncio.wait_for(
            ...
            timeout=60.0
        ) # asyncio.wait_for은 비동기 함수고, 이 함수 안에선 비동기적으로 모든 모델 호출을 동시에 하는 것 -> 또한 이 모든 것을 기다리는 것이 바깥 await
...
async def get_mcp_access_token(
    supabase_token: str,
    base_mcp_url: str,
) -> Optional[Dict[str, Any]]: # # key는 항상 str / value는 아무거나.
    try:
        form_data = {
